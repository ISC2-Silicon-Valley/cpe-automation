...
from config import Config, Google, Meeting
...
class CPECertificateManager:
    ...
    def _get_data(self, sheet_tab_name):
        try:
            return (
                ...
                .values()
                .get(
                    spreadsheetId=self.config.CPECertificateSheetID,
    ...
    def _copy_template(self, user_cpe_file_name):
        try:
            body = {
                "name": user_cpe_file_name,
                "parents": [self.config.CPECertificateFolderID],
            }
            return (
                self.drive_service.files()
                .copy(
                    body=body, fileId=self.config.CPECertificateTemplateID, fields="id"
                )
                .execute()
                .get("id")
            )
    ...
    def _merge_template(self, user_cpe_file_name, merge_data):
        try:
            ...
            context = (
                merge_data.iteritems()
                if hasattr({}, "iteritems")
                else merge_data.items()
            )
            ...
            self.docs_service.documents().batchUpdate(
                body={"requests": reqs}, documentId=user_cpe_file_id, fields=""
            ).execute()
    ...
    def SendCPECertificates(self, meeting_date):
        ...
        for i, row in enumerate(data):
            ...
            user_cpe_file_name = "%s_CPE_%s" % (
                dt.strptime(merge_data["Meeting_Date"], "%m/%d/%Y").strftime("%Y %b"),
                merge_data["member_name"],
            )
            user_cpe_file_name = user_cpe_file_name.upper().replace(" ", "_")
            user_cpe_file_id = self._merge_template(user_cpe_file_name, merge_data)

            user_cpe_pdf_file_name = "%s%s.pdf" % (
                self.config.OutputPath,
                user_cpe_file_name,
            )

            self.g.download_file(
                user_cpe_file_id, user_cpe_pdf_file_name, "application/pdf"
            )
            self.m.send_message_and_attachments(
                merge_data["member_name"],
                merge_data["email_address"],
                user_cpe_pdf_file_name,
                meeting_date,
            )
...
meeting_date = pd.to_datetime("12/07/2023")
